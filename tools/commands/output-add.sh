...
# Output file management - Add output files to system
# This script handles adding output files from source to target directory with atomic copying
...
process_output_files() {
    ...
    log_message "Starting output file processing..."
...
show_usage() {
    echo "Usage: rift output-add [options]"
    echo
    echo "Add output files from $OUTPUT_SOURCE_DIR to target directory"
    echo
    echo "Options:"
    ...
    echo
    echo "Configuration:"
    echo "  Source directory: $OUTPUT_SOURCE_DIR"
    echo "  Target directory: $OUTPUT_TARGET_DIR"
    echo "  File ownership: $OUTPUT_OWNER_UID:$OUTPUT_OWNER_GID"
    echo "  File permissions: $OUTPUT_PERMISSIONS"
...
# Main function to handle output-add command
handle_output_add_command() {
    ...
    if process_output_files; then
        log_message "Output file addition completed successfully"
        return 0
    else
        log_message "Output file addition completed with errors"
        return 1
    fi
}
